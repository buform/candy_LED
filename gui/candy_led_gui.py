...
def select_device(): # menu wybierania urządzenia LED
    ...
    def save_device_path():
        ...
        if devpath:
            ...
            tk.messagebox.showerror("Error", "Device path cannot be empty.") # ścieżka do urządzenia nie może być pusta
...
def create_main_menu():
    ...
    title_label = tk.Label(
		main_window,
		text="Before turning on your LED, please first save the path to the device!\nconst - diode lighting up\nblink - diode blinking",
		font=("", 12, "bold"),
		pady=10	
	)
    title_label.pack()
    ...
    off_button = tk.Button(
		main_window,
		text="TURN OFF",
		command=lambda: changeColor('black'),
		bg="black",
		fg="white"
    )
    off_button.pack(pady=10, padx=10, fill=tk.X)
...
def create_color_menu(main_window):
    ...
    back_button = tk.Button(
        color_window,
        text="BACK",
        command=lambda: color_window.destroy(),
        bg="grey",
        fg="white"
    )
